...
# FastAPI application's base URL
...
async def fetch_manga(server='MANGANELO', genre="", page=1):
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"{Fore.RED}Failed to fetch manga list for genre '{genre}' at page {page}")
        return []
...
async def fetch_manga_details(manga_id, server='MANGANELO'):
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"{Fore.RED}Failed to fetch details for manga ID {manga_id}")
        return {}
...
def append_to_csv(df, filename='./files/mangas_embedd.csv'):
...
async def main():
    ...
    processed_manga_ids, checkpoint = set(
    ), {'genre_index': 0, 'page': 1, 'processed_manga_ids': []}
    try:
        with open(checkpoint_file) as file:
            checkpoint = json.load(file)
            processed_manga_ids.update(
                checkpoint.get('processed_manga_ids', []))
    except FileNotFoundError:
        logger.info(
            f"{Fore.YELLOW}Checkpoint file not found, starting from scratch.")
    ...
    for genre_index, genre in enumerate(GENRES[checkpoint['genre_index']:], start=checkpoint['genre_index']):
        ...
        async for page in tqdm(range(checkpoint['page'], 51), desc=f"{genre}", leave=True):
            ...
            new_data_df = pd.DataFrame(
                current_mangas).drop_duplicates(subset=['id'])
            append_to_csv(new_data_df)
            checkpoint.update({'genre_index': genre_index, 'page': page + 1,
                              'processed_manga_ids': list(processed_manga_ids)})
