...
mangas['authors'] = mangas['authors'].astype(str).apply(
    lambda x: x if x != "['']" else '')  # Handle list-like string representation
mangas['genres'] = mangas['genres'].astype(str)
mangas['latestChapter'] = mangas['latestChapter'].astype(str)
mangas['rating'] = mangas['rating'].apply(
    pd.to_numeric, errors='coerce').fillna(0.0)
mangas['views'] = mangas['views'].apply(
...
def generate_vector(manga, attributes):
    ...
    for attribute in attributes:
        if attribute == 'title' or attribute == 'description' or attribute == 'authors' or attribute == 'genres':
            ...
            # Assuming a max chapter count
            vector_parts.append(encode_numeric(chapter_num, 10000))
...
def get_recommendations(query_title, limit=10):
    query_vector = next(row['vector'] for index,
                        row in mangas.iterrows() if row["title"] == query_title)
